...
namespace sqlite
{
    ...
    class exception : public std::exception
    {
        ...
        friend sqlite;
    public:
        exception(std::string msg, std::string sqlite_msg)
					: _msg(msg), _sqlite_msg(sqlite_msg)
        {
        }
        ...
        {
						std::stringstream ss;
						ss << "SQLite error(" << this->_err << ")" << this->_msg<< "\n" << this->_sqlite_msg;
            return ss.str();
        }
    private:
    ...
    public:
        ...
        void prepare()
        {
            ...
            if(rc != SQLITE_OK)
            {
                exception e("Could not prepare sql.", sqlite3_errmsg(this->_db));
                e._set_errorno(rc);
                throw e;
            }
        ...
        void reset()
        {
            ...
            if(rc != SQLITE_OK)
            {
                exception e("Could not reset the virtual machine.", sqlite3_errmsg(this->_db));
                throw e;
            }
            this->_valid = true;
            this->_has_row = false;
        ...
        void bind(int where, std::string text)
        {
            ...
            if(rc != SQLITE_OK)
            {
                exception e("Could not bind text.", sqlite3_errmsg(this->_db));
                throw e;
            }
        }
        void bind(int where, double d)
        {
            ...
            if(rc != SQLITE_OK)
            {
                exception e("Could not bind double.", sqlite3_errmsg(this->_db));
                throw e;
            }
        }
        void bind(int where, int i)
        {
            ...
            if(rc != SQLITE_OK)
            {
                exception e("Could not bind int.", sqlite3_errmsg(this->_db));
                throw e;
            }
        }
        void bind_null(int where)
        {
            ...
            if(rc != SQLITE_OK)
            {
                exception e("Could not bind to NULL.", sqlite3_errmsg(this->_db));
                throw e;
            }
        }
    ...
    public:
        sqlite(std::string filename) throw (exception)
        {
            ...
            if(rc != SQLITE_OK)
            {
                exception e("Could not open '" + filename + "'", sqlite3_errmsg(this->_db));
                throw e;
            }
        }
